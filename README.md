...
- implemented Kernel Density Estimation (KDE) from scratch and used it for foreground–background separation, analyzing the effect of kernel choice and bandwidth on detection quality

- modeled noisy time-series generated by unknown discrete-time dynamical systems, learning data-driven recurrences and evaluating both one-step prediction and long-horizon autoregressive rollouts

- performed real-world time-series forecasting on cumulative GitHub star data, comparing classical statistical models with neural sequence models across multiple forecast horizons

- implemented a Variational Autoencoder (VAE) for image generation, analyzed the reconstruction–regularization trade-off, and studied how latent variance and KL weighting affect sample diversity and quality
